...
export class BotUpdate {
    constructor(private readonly userService:UserService,private playlistService:PLaylistService,private readonly musicService:MusicService) { }
...
/help – Show this help message`);
    ...
    async getUserPlaylisr(@Ctx() ctx:MyContext & {session:any}) {
        ...
        if (!getUserPlaylists) {
            await ctx.reply('Your playlists is empty. Add songs with /addtoplaylist command.');
            return;
        }
    ...
    @On('callback_query')
    async onCallbackQuery(@Ctx() ctx: MyContext & {callbackQuery:any,session:any}) {
        ...
        if (data.startsWith('playlist_id')) {
            ...
            for (const musicId of musics) {
                const { data: musicData } = await this.musicService.getOneByIdMusic(musicId);
                message+=`${count}. ${musicData?.name}\n`
                buttons.push({ text: String(count) || "Unknown", callback_data:`music_id_${musicData?._id}` });
                count+=1
            }
        ...
        if (data.startsWith('music_id')) {
            ...
            ctx.replyWithAudio({ source: fs.createReadStream((music?.url as any)) },{title:music?.name})
        }
        if (data.startsWith('genre_music')) {
            ...
            if (page > 1) {
                buttons[1].push({ text: '⬅️ Previous', callback_data: `music_page_${page - 1}` });
            }
        
            currentlyMusic.forEach((music, i) => {
                buttons[0].push({ text: `${i + 1}`, callback_data: `music_id_${String(music._id)}` });
            });
        
            if (page < totalPages) {
                buttons[1].push({ text: 'Next ➡️', callback_data: `music_page_${page + 1}` });
            }
